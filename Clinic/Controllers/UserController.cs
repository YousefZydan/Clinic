...
namespace Clinic.Controllers
{
    public class UserController(IAccountService _accountService) : ApiController
    {
        ...
        [HttpGet("GetPrescription/{doctorId}")]
        public async Task<IActionResult> GetPrescriptionByDoctorId([FromRoute] Guid doctorId)
        {
            var userId = User.GetUserId();
            if (string.IsNullOrEmpty(userId))
                return Unauthorized("User ID not found in claims.");
            ...
            var result = await _accountService
                .GetPrescriptionByDoctorId(doctorId, userId);
